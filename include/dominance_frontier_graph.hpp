...
template <typename T>
concept ForwEdgeIter = InputEdgeIter<T> &&
                       std::forward_iterator<T>;

namespace rgs = std::ranges;

template <typename T>
class DomJoinGraph final : private DomTreeGraph<T> {
...
public:
  ...
  dumpInDotFormat(std::ofstream &DotDump,
    ...
    DTG::dumpInDotFormatBaseImpl(DotDump, GraphName, NodeShape,
...
template <typename T>
class DomFrontGraph final : public DirectedGraph<T> {
